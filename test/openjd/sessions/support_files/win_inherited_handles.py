...
def get_inherit_handles_for(proc_handle: HANDLE) -> set[HANDLE]:
    ...
    if ret == 0:
        ...
        raise RuntimeError("Failed to fetch handle information")
...
def main() -> None:
    ...
    pph = OpenProcess(PROCESS_QUERY_INFORMATION, False, basic_info.InheritedFromUniqueProcessId)
    ...
    print(
        "Parent Handles with Inherit bit set:", ", ".join(hex(h) for h in parent_inherited_handles)
    )
